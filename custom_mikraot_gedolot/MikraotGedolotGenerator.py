...
class MikraotGedolotGenerator:
    ...
    @staticmethod
    def calculate_pages_per_text(
        text, font_size, aspect_ratio=0.75, page_size=(794 - 166, 1123 - 166)
    ):
        # A4 = (794px, 1123px)
    ...
    def format_commentators(self, chapter, verse):
        ...
        for commentator in self.book_content.commentary[chapter][verse]:
            commentators_for_template.append(
                {"name": commentator.heName, "text": commentator.content}
            )
    ...
    def generate(self):
        ...
        for ch_index, chapter in enumerate(self.book_content.book.content):
            ...
            while v_num < len(chapter):
                ...
                text_p = (
                    self.calculate_pages_per_text(chapter[v_num], 21)
                    + self.calculate_pages_per_text(
                        self.book_content.translation.content[ch_index][v_num], 14
                    )
                ...
                if text_p // 1 < 2 and text_p % 1 < 0.6 and len(cache["content"]) <= 3:
                    ...
                    cache["translation"].append(
                        self.book_content.translation.content[ch_index][v_num]
                    )
                    ...
                    self.template_content.append(
                        {
                            ...
                            "book": "{}".format(self.book_content.book.heName),
                            "chapter": "{}".format(ch_num),
            ...
            if len(cache["verse"]) > 0:
                ...
                self.template_content.append(
                    {
                        "verse": "{}".format(
                            cache["verse"][0] if len(cache["verse"]) > 0 else v_num + 1
                        ),
                        "book": "{}".format(self.book_content.book.heName),
                        "chapter": "{}".format(ch_num),
                        "content": " ".join(cache["content"]),
        ...
        return {
            ...
            "book_info": {
                ...
                "trans": (
                    self.book_content.translation.version
                    if self.book_content.translation.version is not None
...
class TemplateManager:
    ...
    def render_template(self, data):
        ...
        with open(
            "generated/{}/html/rendered_html.html".format(self.output_file), "wb"
        ) as f:
        ...
        subprocess.Popen(
            [
                "prince",
                "generated/{}/html/rendered_html.html".format(self.output_file),
                "-s",
                "templates/styles.css",
                "-o",
                "generated/{}/pdf/out.pdf".format(self.output_file),
            ]
        )
...
if __name__ == "__main__":
    book_content = BookContent(
        "Leviticus", "Onkelos Leviticus", ["Rashi on Leviticus"], text_range=(1, 2)
    )
