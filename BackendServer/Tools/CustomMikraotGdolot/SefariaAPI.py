...
class SefariaAPI:
    ...
    def chapters_in_book(self, book, lang=None, version=None):
        book = self.get_book_text(book, lang=lang, version=version)
        for chapter in range(len(book[0])):
            yield book[0][chapter], book[1][chapter]

    def get_book_text(self, book, lang=None, version=None):
        params = ''
        if version is not None:
            params = '/{lang}/{ver}?pad=0'.format(lang=lang, ver=version)
